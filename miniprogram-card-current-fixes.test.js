...
test('catalog enum caches are invalidated by remote manifest versions', () => {
  ...
  assert.strictEqual(manifest.cardCatalogOptions.version, '2026-05-16-card-taxonomy-v3')
...
test('repurchase keeps series linkage and skips stale series image sync warnings', () => {
  ...
  assert(wxml.includes('wx:if="{{!child.hasSeriesImageLink}}"'), 'grouped cards should show link action when they lack a concrete series image')
  assert(wxml.includes('wx:if="{{!item.hasSeriesImageLink}}"'), 'single cards should show link action when they lack a concrete series image')
  assert(detailWxml.includes('wx:if="{{!item.hasSeriesImageLink}}"'), 'series detail cards should show link action when they lack a concrete series image')
})
