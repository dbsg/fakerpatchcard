...
const DESCRIPTION = "2014-15 Panini Flawless 高端名宿双物料签字系列，国内常被称为“腰子”。卡面为 on-card auto + dual memorabilia 设计，收录 Kobe Bryant、Larry Bird、Shaquille O'Neal、Hakeem Olajuwon 等传奇与球星，并包含 Base、Ruby、Gold、Emerald、Black 多个平行版本。"
const CREATOR_OPENID = 'oaQG05Ax1I_QRhOkD4lwqOXEucV8'

const PLAYERS = [
  ['GDM-AD', 'Adrian Dantley'],
  ['GDM-BK', 'Bernard King'],
  ['GDM-CD', 'Clyde Drexler'],
  ['GDM-CP', 'Chris Paul'],
  ['GDM-DR', 'David Robinson'],
  ['GDM-DW', 'Dominique Wilkins'],
  ['GDM-GH', 'Grant Hill'],
  ['GDM-GP', 'Gary Payton'],
  ['GDM-HKO', 'Hakeem Olajuwon'],
  ['GDM-JS', 'John Stockton'],
  ['GDM-KB', 'Kobe Bryant'],
  ['GDM-KM', 'Karl Malone'],
  ['GDM-LB', 'Larry Bird'],
  ['GDM-RA', 'Ray Allen'],
  ['GDM-RH', 'Robert Horry'],
  ['GDM-RP', 'Robert Parish'],
  ['GDM-RS', 'Ralph Sampson'],
  ['GDM-RW', 'Russell Westbrook'],
  ['GDM-SN', 'Steve Nash'],
  ['GDM-SO', "Shaquille O'Neal"]
]
...
const SUBSETS = [
  {
    ...
    exclude: new Set(),
    defaultPrintRun: 25,
    overrides: { 'GDM-DW': 6, 'GDM-RH': 10, 'GDM-RW': 0, 'GDM-SN': 0 }
  },
  {
    name: 'Ruby',
    order: 1,
    exclude: new Set(['GDM-DW']),
    defaultPrintRun: 15,
    overrides: { 'GDM-RW': 0, 'GDM-SN': 0 }
  },
  {
    name: 'Gold',
    order: 2,
  ...
  },
  {
    ...
    exclude: new Set(),
    defaultPrintRun: 5,
    overrides: { 'GDM-DW': 3, 'GDM-RW': 0, 'GDM-SN': 0 }
  },
  {
    name: 'Black',
    order: 4,
...
function createItemId(subsetName, code) {
  return `gdma_${subsetName.toLowerCase()}_${code.toLowerCase().replace(/[^a-z0-9]+/g, '_')}`
}

function buildItem(subsetName, code, player, printRun) {
  const item = {
    itemId: createItemId(subsetName, code),
    text: `${code} ${player}`,
    subset: subsetName,
  ...
  item.completionTarget = printRun || 1
  if (printRun) item.printRun = printRun
  return item
}

function buildSubsetDocs(seriesId = '__DRY_RUN_SERIES_ID__') {
  return SUBSETS.map(config => {
    const items = PLAYERS
      .filter(([code]) => !config.exclude.has(code))
      .map(([code, player]) => {
        const printRun = Object.prototype.hasOwnProperty.call(config.overrides, code)
          ? config.overrides[code]
          : config.defaultPrintRun
        return buildItem(config.name, code, player, printRun)
      })
    return {
      seriesId,
      subset: config.name,
...
function buildSeriesDoc(subsetDocs) {
  ...
  return {
    ...
    presetNumbers: [
      ...
      { value: '/6', creatorOpenid: CREATOR_OPENID },
...
function summarize(subsetDocs) {
  ...
  return {
    ...
    subsets: subsetDocs.map(doc => ({
      ...
      noPrintRun: doc.items.filter(item => !item.printRun).map(item => item.text)
    }))
  }
}
...
async function main() {
  ...
  if (existing.length > 0) {
    throw new Error(`同名图鉴已存在，停止创建: ${existing.map(item => item._id).join(', ')}`)
  }
